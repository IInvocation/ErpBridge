...
public abstract class MosaikMappingModelAdapter<TModel> : MosaikModelAdapter<TModel>
{
    ...
    protected override string GetGetByKeyStatement(string primaryKey)
    {
        var sb = new StringBuilder();
        ...
        sb.Append($" FROM {TableName}");
        sb.Append($" WHERE [Nummer] = '{primaryKey}'");

        return sb.ToString();
    }

    protected virtual void Update(string primaryKey, TModel model)
    {
        var sb = new StringBuilder();
